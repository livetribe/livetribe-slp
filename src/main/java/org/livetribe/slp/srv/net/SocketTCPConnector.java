...
public class SocketTCPConnector implements TCPConnector
{
    ...
    {
        if (settings.containsKey(TCP_MESSAGE_MAX_LENGTH_KEY)) setTCPMessageMaxLength(settings.get(TCP_MESSAGE_MAX_LENGTH_KEY));
    }
    ...
    public byte[] read(Socket socket) throws EOFException, SocketTimeoutException
    {
        try
        {
            ...
            if (length > maxLength) throw new IOException("Message length " + length + " is greater than max allowed message length " + maxLength);
