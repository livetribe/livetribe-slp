...
public class AttributeListExtension extends Extension
{
    ...
    protected void deserializeBody(byte[] bodyBytes) throws ServiceLocationException
    {
        ...
        offset += URL_LENGTH_BYTES_LENGTH;
        setURL(readString(bodyBytes, offset, urlLength));

        offset += urlLength;
        int attributesLength = readInt(bodyBytes, offset, ATTRIBUTES_LENGTH_BYTES_LENGTH);

        offset += ATTRIBUTES_LENGTH_BYTES_LENGTH;
        setAttributes(new Attributes(readString(bodyBytes, offset, attributesLength)));
    ...
    public static List findAll(Collection extensions)
    {
        ...
        {
            Extension extension = (Extension)allExtensions.next();
            if (ATTRIBUTE_LIST_EXTENSION_ID == extension.getId()) result.add(extension);
        }
        return result;
    }
}
